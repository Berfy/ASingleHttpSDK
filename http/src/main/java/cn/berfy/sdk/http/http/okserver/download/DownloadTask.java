 ...
 */
package cn.berfy.sdk.http.http.okserver.download;
...
import cn.berfy.sdk.http.http.okgo.db.DownloadManager;
import cn.berfy.sdk.http.http.okgo.exception.HttpException;
import cn.berfy.sdk.http.http.okgo.exception.OkGoException;
import cn.berfy.sdk.http.http.okgo.exception.StorageException;
import cn.berfy.sdk.http.http.okgo.model.HttpHeaders;
import cn.berfy.sdk.http.http.okgo.model.Progress;
import cn.berfy.sdk.http.http.okgo.request.base.Request;
import cn.berfy.sdk.http.http.okgo.utils.HttpUtils;
import cn.berfy.sdk.http.http.okgo.utils.IOUtils;
import cn.berfy.sdk.http.http.okhttp.utils.HLogF;
import cn.berfy.sdk.http.http.okserver.OkDownload;
import cn.berfy.sdk.http.http.okserver.task.PriorityRunnable;
...
public class DownloadTask implements Runnable {
    ...
    private final String TAG = Constant.HTTPTAG + "_DownloadTask";
    ...
    public DownloadTask folder(String folder) {
        if (folder != null && !TextUtils.isEmpty(folder.trim())) {
            ...
            HLogF.w(TAG, "folder is null, ignored!");
        }
        return this;
    }
    ...
    public DownloadTask fileName(String fileName) {
        if (fileName != null && !TextUtils.isEmpty(fileName.trim())) {
            ...
            HLogF.w(TAG, "fileName is null, ignored!");
        }
        return this;
    }
    ...
    public void start() {
        ...
        if (progress.status == Progress.NONE || progress.status == Progress.PAUSE || progress.status == Progress.ERROR) {
            ...
            HLogF.w(TAG, "the task with tag " + progress.tag + " is already in the download queue, current task status is " + progress.status);
        }
    }
    ...
    public void pause() {
        ...
        if (progress.status == Progress.WAITING) {
            ...
            HLogF.w(TAG, "only the task with status WAITING(1) or LOADING(2) can pause, current status is " + progress.status);
        }
    }
