 ...
 */
package cn.berfy.sdk.http.http.okserver.upload;
...
import cn.berfy.sdk.http.http.okgo.db.UploadManager;
import cn.berfy.sdk.http.http.okgo.model.Progress;
import cn.berfy.sdk.http.http.okgo.model.Response;
import cn.berfy.sdk.http.http.okgo.request.base.ProgressRequestBody;
...
import cn.berfy.sdk.http.http.okserver.OkUpload;
...
public class UploadTask<T> implements Runnable {
    ...
    private final String TAG = Constant.HTTPTAG + "_UploadTask";
    ...
    public UploadTask<T> start() {
        ...
        if (progress.status != Progress.WAITING && progress.status != Progress.LOADING) {
            ...
            HLogF.w(TAG, "the task with tag " + progress.tag + " is already in the upload queue, current task status is " + progress.status);
        }
        return this;
    }
