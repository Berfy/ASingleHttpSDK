...
public class HttpApiV1 {
    ...
    public static HttpApiV1 mHttpApi;
    ...
    public static HttpApiV1 init(Context context) {
        if (null == mHttpApi) {
            mHttpApi = new HttpApiV1(context);
        }
        return mHttpApi;
    }

    synchronized public static HttpApiV1 getInstances() {
        if (null == mHttpApi) {
            synchronized (HttpApiV1.class) {
                if (null == mHttpApi) {
    ...
    public HttpApiV1 setHost(String baseUrl) {
        HLogF.d(Constant.HTTPTAG, "新的主机名" + baseUrl);
        mBaseUrl = baseUrl;
        return mHttpApi;
    }

    public HttpApiV1 setTimeOut(long connectTimeout, long readTimeout, long writeTimeout) {
    ...
    public HttpApiV1 setHeader(HttpParams headers) {
        mHeaders = headers;
        return mHttpApi;
    }

    public HttpApiV1 setCacheDir(String dir) {
        HLogF.d(Constant.HTTPTAG, "缓存目录" + dir);
        Constant.setCacheDir(dir);
        return mHttpApi;
    }

    public HttpApiV1 setStatusListener(OnStatusListener onStatusListener) {
        mOnStatusListener = onStatusListener;
        return mHttpApi;
    }

    public HttpApiV1 setDebug(boolean isDebug) {
        Constant.DEBUG = isDebug;
        return mHttpApi;
    }

    public HttpApiV1 setLogTAG(String tag) {
    ...
    private void checkResult(Response response, SuperOkHttpCallBack callBack, boolean needGson) {
        ...
        if (response.code() >= 200 && response.code() <= 300) {//状态码
            try {
                ...
                if (null != callBack) {
                    callBack.finishV1(response.code(), resultJson, needGson);
                }
            } catch (Exception e) {
                e.printStackTrace();
                HLogF.d(Constant.HTTPTAG, "返回值解析出错" + e.getMessage());
                if (null != callBack) {
